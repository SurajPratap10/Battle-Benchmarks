...
class DeepgramAura2TTSProvider(TTSProvider):
    ...
    async def generate_speech(self, request: TTSRequest) -> TTSResult:
        ...
        # Add query parameters to URL
        params = {
            "model": request.voice,  # Full voice ID like "aura-2-asteria-en" (this IS the model)
            "encoding": "mp3" if request.format == "mp3" else "linear16"
        }
...
class SarvamBulbulV3TTSProvider(TTSProvider):
    ...
    def __init__(self):
        ...
        # Map voice IDs to Sarvam Bulbul v3 speaker names
        # Available speakers for bulbul:v3: aditya, ritu, ashutosh, priya, neha, rahul, pooja, rohan, 
        # simran, kavya, amit, dev, ishita, shreya, ratan, varun, manan, sumit, roopa, kabir, 
        # aayan, shubh, advait, amelia, sophia, anand, tanya, tarun, sunny, mani, gokul, vijay, 
        # shruti, suhani, mohit, kavitha, rehan, soham, rupali
        self.voice_to_speaker_map = {
            "en-IN-male": "aditya",        # Male English voice (bulbul:v3 compatible)
            "en-IN-female": "priya",       # Female English voice (bulbul:v3 compatible)
            "hi-IN-male": "amit",          # Male Hindi voice (bulbul:v3 compatible)
            "hi-IN-female": "ritu"         # Female Hindi voice (bulbul:v3 compatible)
        }
    ...
    async def generate_speech(self, request: TTSRequest) -> TTSResult:
        ...
        # Get speaker name from voice mapping - CRITICAL for gender selection
        speaker = self.voice_to_speaker_map.get(request.voice, "priya")  # Default to female (bulbul:v3 compatible)
...
class TTSProviderFactory:
    ...
    def create_provider(provider_id: str) -> TTSProvider:
        ...
        elif provider_id == "sarvam_bulbul_v3":
            return SarvamBulbulV3TTSProvider()
        else:
